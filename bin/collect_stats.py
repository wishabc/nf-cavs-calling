...
def main(df: pd.DataFrame, fdr_tr=0.1):
    tested_vars = df['group_id'].value_counts().rename('tested')
    imbalanced_vars = df.query(f'min_fdr <= {fdr_tr}')['group_id'].value_counts().rename('imbalanced')
...
if __name__ == '__main__':
    ...
    res_df = main(aggregated_variants.dropna(subset=['min_fdr']), args.fdr)
