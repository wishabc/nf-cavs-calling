...
def main(df, fdr_tr=0.05):
    ...
    imbalanced_vars = tested_vars.copy().rename(columns={'tested': 'imbalanced'})
    imbalanced_vars['imbalanced'] = 0#df[df['min_fdr'] <= fdr_tr]['group_id'].value_counts().rename('imbalanced')
